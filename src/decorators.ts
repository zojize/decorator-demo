...
export function validatorDecoratorFactory<T>(
  ...
  return (target, key, index) => {
    ...
    const paramValidators =
      methodValidators.get(key) ?? methodValidators.set(key, []).get(key)!;
    (paramValidators[index] ?? (paramValidators[index] = [])).push(validator);
  };
}
...
export function ref(): PropertyDecorator {
  return (target, key) => {
    ...
    Reflect.defineProperty(target, key, {
      get: () => _ref?.value,
      set: (value: any) =>
        void (console.log(value, _ref),
        ((_ref ?? (_ref = ref_())).value = value)),
      configurable: true,
      enumerable: true,
    });
  };
